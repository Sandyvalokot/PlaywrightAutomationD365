...
test('Create PO', async ({ page }) => {
  ...
  await page.goto('/?cmp=lmuk&mi=DefaultDashboard');
...
test.setTimeout(380_000);
  ...
  await page.getByRole('button', { name: 'Skip setup' }).hover();
  await page.getByRole('button', { name: 'Skip setup' }).click();
  await page.getByRole('button', { name: 'Yes' }).hover();
...
//  await page.getByRole('button', { name: 'Yes' }).click();
  ...
  await page.getByRole('treeitem', { name: 'Modules' }).locator('span').nth(1).hover();
  await page.getByLabel('Modules').locator('span').nth(1).click();

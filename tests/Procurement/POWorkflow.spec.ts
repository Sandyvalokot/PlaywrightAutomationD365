...
test.setTimeout(380_000);
 ...
 // await page.getByText('Don\'t show this again').click();
  ...
  //-----------------------
 // await page.getByRole('button', { name: 'Administration' }).hover();
 // await page.getByRole('button', { name: 'Administration' }).click();
 // await page.getByRole ('button', { name: 'Expand all' }).click();
 // Wait for the "Expand all" button to appear and become visible, then click it
  

  //await page.getByRole('combobox', { name: 'Buyer group' }).hover();
  //await page.getByRole('combobox', { name: 'Buyer group' }).click();
 // await page.locator('#PurchCreateOrder_26_groupAdministraton_ItemBuyerGroupId div').nth(1).click();
  //await page.getByRole('combobox', { name: 'Buyer group' }).fill('MKT');
  //await page.getByTitle('MKT', { exact: true }).click();
 // await page.getByRole('combobox', { name: 'Buyer group' }).fill('MKT');
  

// --sELECT lINE ITEM
 await page.getByRole('row', { name: 'The row is up to date. Type' }).getByLabel('Procurement category').click();
 ...
 await page.getByRole('row', { name: 'The row is up to date. Type' }).getByLabel('Procurement category').press('Enter');
  ...
  await page.getByRole('textbox', { name: 'Quantity' }).click();
    ...
    await page.getByRole('textbox', { name: 'Unit price', exact: true }).click();
    await page.getByRole('textbox', { name: 'Unit price', exact: true }).fill('100');
    await page.getByRole('textbox', { name: 'Unit price', exact: true }).press('Tab');
    await page.waitForTimeout(6000);

  // select line details
  await page.getByRole('button', { name: 'Line details' }).hover();
  ...
  await page.getByRole('tab', { name: 'Financial dimensions' }).locator('span')
    .click();
  await page.waitForTimeout(6000);
  ...
  await page.getByRole('combobox', { name: 'EventType value' }).click();
    ...
    // fill the hearder details
    await page.getByText('Header', { exact: true }).click();
    await page.waitForTimeout(3000);
    //await page.getByRole('heading', { name: 'Administration' }).hover();
    //await page.getByRole('button', { name: 'Administration' }).click();
    //await page.getByRole('button', { name: 'Expand all' }).click();
  ...
  await page.getByRole('button', { name: ' Workflow' })
   ...
   // Receive the purchase order
    ...
    await page.getByRole('textbox', { name: 'Product receipt' }).fill('Test Receipt1');
